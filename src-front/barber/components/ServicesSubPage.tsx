...
interface ServicesSubPageProps {
  ...
  onSave: (newServices: Service[]) => void;
  onBack: () => void;
}

const ServicesSubPage: React.FC<ServicesSubPageProps> = ({ initialServices, onSave, onBack }) => {
  ...
  const handleDeleteService = () => {
    ...
    window.showToast('خدمت با موفقیت حذف شد.', 'info');
  ...
  return (
    <div className="absolute inset-0 bg-surface-1 z-50 flex flex-col">
      <header className="sticky top-0 bg-surface-1/80 backdrop-blur-sm p-4 z-50 flex items-center gap-4 border-b border-gray-200">
        <button onClick={handleSaveAndBack} aria-label="بازگشت"><ArrowRight /></button>
                    ...
                    <div className="flex items-center gap-4">
                         <div className="w-24 h-24 rounded-lg bg-surface-2 flex items-center justify-center overflow-hidden">
                            {formData.sampleImage ? (
                                <img src={formData.sampleImage} alt="نمونه کار" className="w-full h-full object-cover" />
                            ) : (
