...
import type { BarberContextType, Schedule } from '../types';
...
export const SchedulePage: React.FC<SchedulePageProps> = ({ context }) => {
    const [editedSchedule, setEditedSchedule] = useState<Schedule[]>(JSON.parse(JSON.stringify(MOCK_SCHEDULE)));
    ...
    const handleSave = () => {
        context.showToast('ساعات کاری با موفقیت ذخیره شد.', 'success');
    ...
    return (
        <div className="flex flex-col h-full bg-surface-1">
             <header className="sticky top-0 bg-surface-1/80 backdrop-blur-sm p-4 z-50 flex items-center gap-4 border-b border-gray-200">
                <button onClick={handleBack} aria-label="بازگشت"><ArrowRight /></button>
            ...
            <footer className="p-4 border-t border-gray-200 flex gap-2" style={{paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))'}}>
                <button onClick={handleBack} className="flex-1 h-12 flex items-center justify-center gap-1 bg-gray-200 text-gray-800 px-3 py-1.5 rounded-md font-semibold transition hover:bg-gray-300">
                    <X size={18} /> لغو
                </button>
