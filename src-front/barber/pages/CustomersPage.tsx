...
import type { BarberContextType, Customer } from '../types';
...
export const CustomersPage: React.FC<CustomersPageProps> = ({ context }) => {
    const [selectedCustomer, setSelectedCustomer] = useState<Customer | null>(null);
    const [searchTerm, setSearchTerm] = useState('');
    const [zoomedAvatar, setZoomedAvatar] = useState<string | null>(null);

    useEffect(() => {
        const customerId = context.pageParams?.customerId;
        if (customerId) {
    ...
    const handleBackFromDetail = () => {
        ...
        if (context.setSelectedCustomer) {
            context.setSelectedCustomer(null);
        }
    }
                ...
                <main className="flex-grow p-4 space-y-3">
                    {filteredCustomers.map(customer => (
                        <div key={customer.id} onClick={() => {
                            setSelectedCustomer(customer);
                            if (context.setSelectedCustomer) {
                                context.setSelectedCustomer(customer);
                            }
                        }} className="w-full text-right flex items-center justify-between p-3 bg-white rounded-lg border shadow-xs cursor-pointer hover:bg-gray-50 transition">
...
const CustomerDetail: React.FC<{ customer: Customer; onBack: () => void }> = ({ customer, onBack }) => {
    ...
    const handleManageLoyalty = () => {
       if (typeof window !== 'undefined' && (window as any).showToast) {
           (window as any).showToast("برای مدیریت امتیازات لطفا به بخش 'باشگاه مشتریان' مراجعه کنید.", 'info');
       }
    };
