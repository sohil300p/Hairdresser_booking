...
import type { BarberContextType, Appointment } from '../types';
...
export const AppointmentsPage: React.FC<AppointmentsPageProps> = ({ context }) => {
    ...
    const handleReservationAction = (id: number, action: 'confirm' | 'reject') => {
        const appointment = appointments.find(r => r.id === id);
        if (!appointment) return;

        if (action === 'confirm') {
            setAppointments(appointments.map(r => r.id === id ? { ...r, status: 'confirmed' } : r));
            context.showToast(`رزرو ${appointment.name} تایید شد.`, 'success');
        } else {
            setAppointments(appointments.map(r => r.id === id ? { ...r, status: 'cancelled' } : r));
            context.showToast(`رزرو ${appointment.name} رد شد.`, 'info');
        }
    };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <main className="flex-grow overflow-y-auto p-4">
        {viewMode === 'list' ? (
            <ListView appointments={filteredAppointments} onAction={handleReservationAction} />
        ) : (
            <CalendarView appointments={appointments} onAction={handleReservationAction} />
        )}
      </main>

      <button 
        type="button"
        onClick={() => context.setCurrentPage('add-reservation')}
...
const ListView: React.FC<{appointments: Appointment[], onAction: (id: number, action: 'confirm' | 'reject') => void}> = ({ appointments, onAction }) => (
    <div className="space-y-3 pb-20">
        {appointments.map(apt => <AppointmentCard key={apt.id} {...apt} onAction={onAction} />)}
...
const CalendarView: React.FC<{appointments: Appointment[], onAction: (id: number, action: 'confirm' | 'reject') => void}> = ({ appointments, onAction }) => {
    ...
    const bookingsByDay = appointments.reduce((acc, curr) => {
    ...
    const appointmentsForSelectedDay = selectedDayNumber ? bookingsByDay[selectedDayNumber] || [] : [];
                ...
                <div className="animate-slide-up">
                    ...
                    <div className="space-y-3">
                        {appointmentsForSelectedDay.length > 0 ? (
                            appointmentsForSelectedDay.map(apt => <AppointmentCard key={apt.id} {...apt} onAction={onAction} />)
                        ) : (
...
const AppointmentCard: React.FC<Appointment & { onAction: (id: number, action: 'confirm' | 'reject') => void }> = ({ id, name, service, time, avatar, status, date, onAction }) => {
    return (
        <div className="bg-white p-4 rounded-lg border shadow-xs">
